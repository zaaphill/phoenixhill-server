...
def _asset_dir():
    """Return the root directory where bundled assets live."""
...
def _set_png_cursor(win, png_name, hotspot_x=0, hotspot_y=0):
    ...
    png_path = os.path.join(_asset_dir(), png_name)
...
class MyGame(ShowBase, BrickMixin, PickingMixin, UIMixin, CharacterMixin, CameraMixin, ShadowMixin, UIDebugMixin, SkyMixin, LoginScreenMixin, CloudMixin, MultiplayerMixin, AvatarMixin):
    def __init__(self):
        ...
        _icon_path = os.path.join(_asset_dir(), 'PiePlex logo.ico')
        ...
        updater.start_update_check(self)
    ...
    def _toggle_fullscreen(self):
        import ctypes
        props = WindowProperties()
        if self.win.getProperties().getFullscreen():
