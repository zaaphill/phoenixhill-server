...
class CharacterMixin:
    def setup_character(self):
        ...
        _asset_base = getattr(_sys, '_MEIPASS', None) if getattr(_sys, 'frozen', False) else _os.getcwd()
        if _asset_base is None:
            _asset_base = _os.getcwd()
        _face_dir = _os.path.join(_asset_base, 'textures', 'face sprites')
        self._face_textures = []
        ...
        self._default_face_textures = list(self._face_textures)
    ...
    def updateMovement(self, task):
        ...
        if not getattr(self, '_respawning', False):
            ...
            for brick in self._grid_nearby(current_pos, 12):
                if brick in getattr(self, 'brick_kill_bricks', set()):
                    ...
                    # Use a generous Z tolerance so landing on top also triggers death
                    if (abs(char_box['center'].x - brick_box['center'].x) < (char_box['half_width']  + brick_box['half_width'])  and
                            abs(char_box['center'].y - brick_box['center'].y) < (char_box['half_depth']  + brick_box['half_depth'])  and
                            abs(char_box['center'].z - brick_box['center'].z) < (char_box['half_height'] + brick_box['half_height'] + 0.15)):
                        self._trigger_respawn()
                        return Task.cont
        ...
        if move_x != 0 or move_y != 0:
            ...
            if move_vec.length() > 0:
                move_vec.normalize()
                total = move_vec * self.move_speed * dt
                # 4 substeps so thin geometry (decorations, corner seams,
                # intersecting parts) is never thinner than one step's travel.
                pos = current_pos
                for _ in range(4):
                    self.character.setPos(pos)
                    sub = Vec3(pos.x + total.x * 0.25,
                               pos.y + total.y * 0.25,
                               pos.z)
                    pos = self.resolve_movement(sub)
                resolved_pos = pos
        ...
        if not self.is_jumping and self.vertical_speed <= 0:
            ...
            _no_col = getattr(self, 'brick_no_collision', set())
            for brick in self._grid_nearby(current_pos, 15):
                ...
                # Use the brick's own footprint + a small tolerance for the XY
                # overlap test (not the full char half-width). This prevents wall
                # bricks that merely graze the player's wide AABB from being
                # counted as step-up ground, which causes z-drift in dense builds.
                if (char_box['min_z'] + 0.05 < brick_top <= char_box['min_z'] + MAX_STEP and
                        abs(cx - brick_box['center'].x) < brick_box['half_width'] + 0.5 and
                        abs(cy - brick_box['center'].y) < brick_box['half_depth'] + 0.5):
        ...
        # Surface-contact kill check — catches landing on top this exact frame.
        if not getattr(self, '_respawning', False):
            feet_z = self.character.getPos().z
            cpos   = self.character.getPos()
            cbox   = self.get_character_collision_box(cpos)
            for brick in self._grid_nearby(cpos, 12):
                if brick not in getattr(self, 'brick_kill_bricks', set()):
                    continue
                bb = self.get_brick_collision_box(brick)
                if (abs(feet_z - bb['max_z']) < 0.12 and
                        abs(cbox['center'].x - bb['center'].x) < (cbox['half_width']  + bb['half_width']) and
                        abs(cbox['center'].y - bb['center'].y) < (cbox['half_depth']  + bb['half_depth'])):
                    self._trigger_respawn()
                    return Task.cont
            ...
            for brick in self._grid_nearby(pos, 12):
                ...
                if abs(hpr.x) > 0.5 or abs(hpr.y) > 0.5 or abs(hpr.z) > 0.5:
                    if not self._obb_aabb_collide(char_box, brick):
                        continue
                    # Only use surface-z logic for bricks whose local up-axis points
                    # mostly upward (slopes/ramps). Wall planks (local Z horizontal)
                    # get a horizontal OBB push directly — not an upward climb.
                    brick_mat   = brick.getMat()
                    local_up_z  = brick_mat.xformVec(Vec3(0, 0, 1)).z
                    surf_z = None
                    if local_up_z >= 0.3:
                    ...
                    if surf_z is not None:
                        ...
                        if push_up <= 1.2 and push_up < best_push_dist:
                            best_push_dist = push_up
                            best_push = Vec3(0, 0, push_up)
                        else:
                            # Surface too high — treat as wall, push horizontally.
                ...
                # Only allow upward push if the brick's top is within step height
                # above the player's feet.  A larger oz_up means the brick is a
                # wall — force a horizontal push instead so we don't climb walls.
                if oz_up > 1.2 and oz_up <= oz_dn:
                    effective_oz = float('inf')
                else:
                    effective_oz = oz

                # For steppable bricks (top within step height), always prefer
                # the upward push over horizontal. Without this, a tiny horizontal
                # clip (ox/oy < oz_up) wins and the player gets pushed sideways
                # while the step-up pass simultaneously tries to lift them,
                # causing jitter on certain approach angles.
                if effective_oz != float('inf') and 0 < oz_up <= 1.2:
                    min_o = effective_oz
                else:
                    min_o = min(ox, oy, effective_oz)
                if min_o < best_push_dist:
                    best_push_dist = min_o
                    if min_o == effective_oz and effective_oz != float('inf'):
                        if oz_up <= oz_dn:
    ...
    def apply_tshirt(self, image_b64):
        ...
        try:
            ...
            pass
        self._nav_av_apply_tshirt(image_b64)
    ...
    def remove_tshirt(self):
        ...
        self._nav_av_apply_tshirt(None)
    ...
    def apply_shirt(self, image_b64):
        ...
        try:
            ...
            pass
        self._nav_av_apply_shirt(image_b64)
    ...
    def _sdbg_print(self):
        print('\n=== SHIRT DEBUG REGIONS (copy into _SHIRT_REGIONS) ===')
    ...
    def _pdbg_print(self):
        print('\n=== PANTS DEBUG REGIONS (copy into _PANTS_REGIONS) ===')
    ...
    def apply_face(self, frames_b64):
        ...
        _nfspr = getattr(self, '_nav_av_face_sprite', None)
        if _nfspr and not _nfspr.isEmpty() and textures:
            _nfspr.setTexture(textures[0])
    ...
    def remove_face(self):
        ...
        if textures:
            ...
            if spr and not spr.isEmpty():
                spr.setTexture(textures[0])
        _nfspr = getattr(self, '_nav_av_face_sprite', None)
        _nftex = getattr(self, '_face_textures', [])
        if _nfspr and not _nfspr.isEmpty() and _nftex:
            _nfspr.setTexture(_nftex[0])
    ...
    def apply_hat(self, hat_data_json):
        ...
        try:
            ...
            self._equipped_hat_x_off = float(data.get("x_offset", 0.0))
            self._equipped_hat_y_off = float(data.get("y_offset", 0.0))
            ...
            if tmp_dir:
                shutil.rmtree(tmp_dir, ignore_errors=True)
        self._nav_av_apply_hat(hat_data_json)
    ...
    def _equipped_hat_follow(self, task):
        ...
        x   = getattr(self, '_equipped_hat_x_off', 0.0)
        y   = getattr(self, '_equipped_hat_y_off', 0.0)
        m.setPos(hp.x + x, hp.y + y, hp.z + 0.55 + z)
