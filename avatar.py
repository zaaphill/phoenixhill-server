...
class AvatarMixin:
    ...
    def _on_avatar_shirt_equip(self, item, _ac, _thr):
        ...
        if item_id == equipped_id:
            ...
            if token:
                _thr.Thread(target=lambda t=token, iid=item_id: _ac.equip_shirt(t, iid), daemon=True).start()
            ws   = getattr(self, "_ws", None)
            loop = getattr(self, "_mp_loop", None)
            if ws and loop and not loop.is_closed():
                import asyncio, json
    ...
    def _on_avatar_pants_equip(self, item, _ac, _thr):
        ...
        if item_id == equipped_id:
            ...
            if token:
                _thr.Thread(target=lambda t=token, iid=item_id: _ac.equip_pants(t, iid), daemon=True).start()
            ws   = getattr(self, "_ws", None)
            loop = getattr(self, "_mp_loop", None)
            if ws and loop and not loop.is_closed():
                import asyncio, json
